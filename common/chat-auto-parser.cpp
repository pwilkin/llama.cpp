...
DiscoveredPattern TemplateAnalyzer::analyze_by_differential(const minja::chat_template& tmpl) {
    ...
    try {
        ...
        LOG_DBG("Tool1 diff: '%s'", tool1_diff.c_str());
        LOG_DBG("Tool2 diff: '%s'", tool2_diff.c_str());
        LOG_DBG("Tool3 diff: '%s'", tool3_diff.c_str());
        ...
        if (tool1_diff.empty() && tool2_diff.empty() && tool3_diff.empty()) {
            ...
            std::string nemotron_diff_false = find_string_difference(nemotron_base_output_false, nemotron_tool_output_false);
            ...
            std::string nemotron_diff_true = find_string_difference(nemotron_base_output_true, nemotron_tool_output_true);
            ...
            if (!nemotron_diff_false.empty()) {
                ...
                std::string nemotron_diff_cross = find_string_difference(nemotron_base_output_false, nemotron_base_output_true);
                LOG_ERR("Nemotron cross diff (gen_prompt=false vs gen_prompt=true, base): %s", nemotron_diff_cross.c_str());
        ...
        if (detected_format != TemplatePattern::UNKNOWN) {
            ...
            if (!has_meaningful_markers) {
                LOG_DBG("Detected format %d but no meaningful tool call markers found - falling back to generic parser", detected_format);
        ...
        if (detected_format == TemplatePattern::UNKNOWN) {
            if (!tool1_diff.empty()) {
                ...
                if (tool1_diff.find("{\"tool_calls\"") != std::string::npos ||
                    ...
                    // Set minimal JSON markers for actual tool calls
                    if (tool1_diff.find("{\"tool_calls\"") != std::string::npos) {
...
DiscoveredPattern TemplateAnalyzer::extract_patterns_from_differences(
    ...
    LOG_DBG("Function name positions - func1_pos: %zu, func2_pos: %zu, func3_pos1: %zu, func3_pos2: %zu", 
            func1_pos, func2_pos, func3_pos1, func3_pos2);
    ...
    if (func1_pos != std::string::npos && func2_pos != std::string::npos) {
            ...
            if (param2_pos != std::string::npos && value2_pos != std::string::npos) {
                if (param2_pos > end_of_val1) {
                    ...
                    if (!patterns.parameter_key_prefix.empty() && 
                        ...
                        gap.substr(gap.length() - patterns.parameter_key_prefix.length()) == patterns.parameter_key_prefix) {
                        
                        std::string closer_and_sep = gap.substr(0, gap.length() - patterns.parameter_key_prefix.length());
        ...
        std::string func_context = tool1_diff;
        ...
        if (open_pos != std::string::npos && open_pos < func1_pos) {
            ...
            for (int i = (int)func1_pos - 1; i >= 0; i--) {
                if (func_context[i] == '{' || func_context[i] == '[' || func_context[i] == '(' || func_context[i] == '<') {
                    start_pos = i;
        ...
        if (patterns.function_opener.length() > 0 && patterns.tool_call_opener.length() > patterns.function_opener.length()) {
            // Check if tool_call_opener ends with function_opener
            if (patterns.tool_call_opener.rfind(patterns.function_opener) == patterns.tool_call_opener.length() - patterns.function_opener.length()) {
                patterns.tool_call_start_marker = patterns.tool_call_opener.substr(0, patterns.tool_call_opener.length() - patterns.function_opener.length());
            } else {
        ...
        if (!patterns.tool_call_start_marker.empty() && patterns.tool_call_start_marker[0] != '<' && patterns.tool_call_start_marker[0] != '[' && patterns.tool_call_start_marker[0] != '{') {
             if (patterns.tool_call_start_marker.find("TOOL") == 0 || patterns.tool_call_start_marker.find("tool") == 0 || patterns.tool_call_start_marker.find("FUNC") == 0 || patterns.tool_call_start_marker.find("func") == 0) {
                 ...
                 if (patterns.tool_call_start_marker.find('>') != std::string::npos) {
                     ...
                     LOG_DBG("Heuristic: prepended < to tool_call_start_marker: '%s'", patterns.tool_call_start_marker.c_str());
                 }
             }
        }
        ...
        if (!patterns.tool_call_start_marker.empty()) {
            ...
            while (!patterns.tool_call_start_marker.empty() && (patterns.tool_call_start_marker.back() == '\n' || patterns.tool_call_start_marker.back() == '\r')) {
...
std::string TemplateAnalyzer::find_tool_call_end(const std::string& diff, size_t func_pos) {
    ...
    if (diff.length() > 0) {
        if (diff[0] == '[' || diff.find("[") < 5) opener_char = '[';
        else if (diff[0] == '{' || diff.find("{") < 5) opener_char = '{';
        else if (diff[0] == '<' || diff.find("<") < 5) opener_char = '<';
...
std::string TemplateAnalyzer::infer_tool_call_opener(const std::string& diff1, const std::string& diff2, const std::string& diff3) {
...
std::string TemplateAnalyzer::infer_tool_call_closer(const std::string& diff1, const std::string& diff2, const std::string& diff3) {
...
std::string TemplateAnalyzer::find_common_start_pattern(const std::string& diff1, const std::string& diff2, const std::string& diff3) {
...
std::string TemplateAnalyzer::find_common_end_pattern(const std::string& diff1, const std::string& diff2, const std::string& diff3) {
...
TemplatePattern::ToolCallFormat TemplateAnalyzer::determine_format_from_patterns(const DiscoveredPattern& patterns) {
    ...
    if (patterns.tool_call_opener.empty() && patterns.tool_call_closer.empty() &&
        patterns.function_opener.empty() && patterns.function_closer.empty() &&
        patterns.parameter_opener.empty() && patterns.parameter_closer.empty() &&
    ...
    if (!patterns.tool_call_opener.empty() &&
        (patterns.tool_call_opener.find("<function=") != std::string::npos ||
    ...
    if (patterns.reasoning_start_marker.empty()) {
        ...
        if (prompt_think != prompt_no_think) {
            ...
            if (!diff.empty()) {
                ...
                if (patterns.reasoning_start_marker[0] == '<') {
                    ...
                    // Try [THINK] -> [/THINK]
                    if (patterns.reasoning_start_marker.front() == '[' && patterns.reasoning_start_marker.back() == ']') {
                         std::string name = patterns.reasoning_start_marker.substr(1, patterns.reasoning_start_marker.length() - 2);
        ...
        if (patterns.reasoning_start_marker.empty()) {
            ...
            if (last_open_angle != std::string::npos && last_close_angle != std::string::npos && last_close_angle > last_open_angle) {
                last_open = last_open_angle;
                last_close = last_close_angle;
            } else if (last_open_bracket != std::string::npos && last_close_bracket != std::string::npos && last_close_bracket > last_open_bracket) {
    ...
    LOG_DBG("Reasoning markers: start='%s', end='%s'", patterns.reasoning_start_marker.c_str(), patterns.reasoning_end_marker.c_str());
}
...
std::vector<std::string> TemplateAnalyzer::extract_preserved_tokens(const minja::chat_template& tmpl) {
    ...
    if (!discovered.tool_call_opener.empty()) tokens.push_back(discovered.tool_call_opener);
    if (!discovered.tool_call_closer.empty()) tokens.push_back(discovered.tool_call_closer);
    if (!discovered.function_opener.empty()) tokens.push_back(discovered.function_opener);
    if (!discovered.function_closer.empty()) tokens.push_back(discovered.function_closer);
    if (!discovered.parameter_opener.empty()) tokens.push_back(discovered.parameter_opener);
    if (!discovered.parameter_closer.empty()) tokens.push_back(discovered.parameter_closer);
    if (!discovered.tool_call_start_marker.empty()) tokens.push_back(discovered.tool_call_start_marker);
    if (!discovered.tool_call_end_marker.empty()) tokens.push_back(discovered.tool_call_end_marker);
    if (!discovered.reasoning_start_marker.empty()) tokens.push_back(discovered.reasoning_start_marker);
    if (!discovered.reasoning_end_marker.empty()) tokens.push_back(discovered.reasoning_end_marker);
...
common_chat_params UniversalPEGGenerator::generate_parser(
    ...
    try {
        ...
        if (data.grammar_lazy) {
             ...
             // Fallback to content-only grammar
             data.grammar = json_schema_to_grammar(json{{"type", "string"}});
        }
...
common_peg_arena UniversalPEGGenerator::build_native_parser(
    const TemplatePattern& pattern,
    const minja::chat_template& tmpl,
    const struct templates_params& inputs,
    ...
    auto parser = build_chat_peg_native_parser([&](common_chat_peg_native_builder & p) {
        ...
        if (pattern.format == TemplatePattern::JSON_NATIVE) {
            ...
            if (!marker.empty() && !pattern.special_markers.at("tool_call_end_marker").empty()) {
                ...
                // If we have a start marker but no end marker, try to infer structure from context
                auto tool_call = p.tool(
                    p.tool_open(p.literal(marker)) +
                    p.space() +
                    // p.tool_name(p.rest()) +
                    p.tool_close(p.eps())  // Empty close for now, will be handled by content parser
                );
                ...
                // If no specific markers found, try to build based on general JSON structure
                // Assume content until '{'
                std::string start_char = "{";
                auto tool_call = p.tool(
                    p.tool_open(p.literal(start_char)) +
                    p.tool_name(p.until(":")) +
                    p.literal(":") +
                    p.tool_args(p.schema(p.json(), "args", json::object())) +
            ...
            std::string marker = pattern.special_markers.at("tool_call_start_marker");
            // If format is not JSON_NATIVE but we have start marker, try to use it
            auto tool_call = p.tool(
                p.tool_open(p.literal(marker)) +
                p.tool_name(p.rest()) +
                p.tool_close(p.eps())
...
common_peg_arena UniversalPEGGenerator::build_constructed_parser(
    const TemplatePattern& pattern,
    const minja::chat_template& tmpl,
    const struct templates_params& inputs,
    bool thinking_forced_open) {
    ...
    auto parser = build_chat_peg_constructed_parser([&](common_chat_peg_constructed_builder & p) {
        ...
        if (pattern.format == TemplatePattern::XML_CONSTRUCTED) {
            ...
            if (!marker.empty() &&
                ...
                if (!pattern.special_markers.at("parameter_key_prefix").empty()) {
                    auto arg = p.tool_arg(
                        ...
                        p.tool_arg_name(p.until(pattern.special_markers.at("parameter_key_suffix").empty() ? ">" : pattern.special_markers.at("parameter_key_suffix"))) +
                        (pattern.special_markers.at("parameter_key_suffix").empty() ? p.eps() : p.literal(pattern.special_markers.at("parameter_key_suffix"))) +
                        p.tool_arg_string_value(p.until(pattern.special_markers.at("parameter_closer").empty() ? "</" : pattern.special_markers.at("parameter_closer"))) +
                        (pattern.special_markers.at("parameter_closer").empty() ? p.eps() : p.tool_arg_close(p.literal(pattern.special_markers.at("parameter_closer"))))
                        // Also consume separator if present
                        + (pattern.special_markers.at("argument_separator").empty() ? p.eps() : p.optional(p.literal(pattern.special_markers.at("argument_separator"))))
                    );
                    
                    args_parser = p.zero_or_more(arg);
                } else {
                ...
                auto function_call_def = p.tool(
                    ...
                    (pattern.special_markers.at("function_opener").empty() ? p.eps() : p.literal(pattern.special_markers.at("function_opener"))) +
                    p.tool_name(p.until(
                        !pattern.special_markers.at("function_name_suffix").empty() ? pattern.special_markers.at("function_name_suffix") :
                        (!pattern.special_markers.at("function_closer").empty() ? pattern.special_markers.at("function_closer") :
                        (pattern.special_markers.at("parameter_key_prefix").empty() ? 
                            (pattern.special_markers.at("parameter_opener").empty() ? ">" : pattern.special_markers.at("parameter_opener")) 
                            : pattern.special_markers.at("parameter_key_prefix"))) 
                    )) +
                    (!pattern.special_markers.at("function_name_suffix").empty() ? p.literal(pattern.special_markers.at("function_name_suffix")) : p.eps()) +
                    ...
                    (pattern.special_markers.at("function_name_suffix").empty() && !pattern.special_markers.at("function_closer").empty() ? p.literal(pattern.special_markers.at("function_closer")) : p.eps()) +
                    ...
                    (!pattern.special_markers.at("function_name_suffix").empty() && !pattern.special_markers.at("function_closer").empty() && pattern.special_markers.at("function_closer") != pattern.special_markers.at("function_name_suffix") ? p.literal(pattern.special_markers.at("function_closer")) : p.eps()) +
            ...
            } else if (!pattern.special_markers.at("function_opener").empty() && !pattern.special_markers.at("function_closer").empty()) {
                ...
                if (!opener.empty() && !closer.empty()) {
                    auto function_call = p.tool(
                        p.tool_open(p.literal(opener)) +
                        p.space() +
                        p.tool_name(p.until(">")) +
                        p.content(p.until(closer)) +
                        p.tool_close(p.literal(closer))
                    );
