...
common_peg_parser_id common_peg_arena::get_rule(const std::string & name) const {
    ...
    if (it == rules_.end()) {
        // This was previously a std::runtime_error, but it's better to return an invalid ID
        // and let the caller handle it, as rules might be optional in some contexts.
        return COMMON_PEG_INVALID_PARSER_ID;
    }
    return it->second;
}
