...
static bool compare_chat_messages(const common_chat_msg& msg1, const common_chat_msg& msg2) {
    ...
    if (tool_calls_equal) {
        for (size_t i = 0; i < msg1.tool_calls.size(); ++i) {
            if (msg1.tool_calls[i].name != msg2.tool_calls[i].name ||
                msg1.tool_calls[i].arguments != msg2.tool_calls[i].arguments ||
                msg1.tool_calls[i].id != msg2.tool_calls[i].id) {
...
static common_chat_msg run_manual_parser(const std::string& input, common_chat_format format, 
                                        common_reasoning_format reasoning_format = COMMON_REASONING_FORMAT_AUTO,
                                        bool parse_tool_calls = true, bool thinking_forced_open = false,
                                        bool reasoning_in_content = false) {
...
static void test_qwen3_coder_comparison(testing& t) {
    ...
    for (const auto& test_case : test_cases) {
        t.test(test_case.name, [&](testing& t) {
            // Run manual parser
            auto manual_result = run_manual_parser(
                test_case.input,
                test_case.manual_format,
                test_case.reasoning_format,
                test_case.parse_tool_calls,
                test_case.thinking_forced_open,
...
static void test_bytedance_seed_oss_comparison(testing& t) {
    ...
    std::vector<ParserComparisonTestCase> test_cases = {
        {
            "ByteDance-Seed-OSS content only",
            "Let me search the knowledge base for cat pictures.",
...
static void test_nvidia_nemotron_nano_v2_comparison(testing& t) {
    ...
    std::vector<ParserComparisonTestCase> test_cases = {
        ...
        },
        {
            ...
            "Let me search the knowledge base for cat pictures."
            "<TOOLCALL>["
            "{\"name\": \"search_knowledge_base\", \"arguments\": {\"query\": \"cat pictures\", \"category\": \"general\"}}"
            "]</TOOLCALL>",
            COMMON_CHAT_FORMAT_NEMOTRON_V2
        },
        {
    ...
    for (const auto& pair : template_test_pairs) {
        t.test("Comprehensive test for " + pair.second, [&](testing& t) {
            ...
            std::vector<std::string> test_inputs = {
                ...
                "<TOOLCALL>["
                "{\"name\": \"search_knowledge_base\", \"arguments\": {\"query\": \"test1\", \"category\": \"general\"}},"
