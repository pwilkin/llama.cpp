...
static void render_all_scenarios(
    const minja::chat_template & tmpl,
    const json & tools,
    bool add_generation_prompt,
    bool enable_thinking,
...
static const char * function_format_to_str(ToolCallStructure::FunctionFormat fmt) {
    switch (fmt) {
        ...
        case ToolCallStructure::FUNC_PREFIXED_INDEXED: return "PREFIXED_INDEXED";
        case ToolCallStructure::FUNC_NAME_AS_KEY:      return "NAME_AS_KEY";
    }
    return "UNKNOWN";
}
...
static const char * argument_format_to_str(ToolCallStructure::ArgumentFormat fmt) {
    switch (fmt) {
        ...
        case ToolCallStructure::ARGS_KEY_VALUE_TAGS: return "KEY_VALUE_TAGS";
    }
    return "UNKNOWN";
}
...
int main(int argc, char ** argv) {
    ...
    LOG_ERR("Options: with_tools=%s, generation_prompt=%s, enable_reasoning=%s\n",
            ...
            opts.generation_prompt ? "true" : "false",
            opts.enable_reasoning ? "true" : "false");
    ...
    try {
        ...
        if (opts.output_mode == OutputMode::ANALYSIS || opts.output_mode == OutputMode::BOTH) {
            ...
            params.reasoning_format = opts.with_deepseek ? COMMON_REASONING_FORMAT_DEEPSEEK : COMMON_REASONING_FORMAT_NONE;
