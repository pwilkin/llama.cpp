...
class peg_test_builder {
  ...
  public:
    ...
    // Execute the test
    void run() { test_peg_parser(tester_.tmpls_.get(), [this](peg_test_case & t) { t = tc_; }); }
};
...
static void test_template_output_parsers() {
    ...
    }
    {
        ...
        auto syntax_no_tools = common_chat_templates_apply(tmpls.get(), inputs_no_tools);
        auto syntax_tools = common_chat_templates_apply(tmpls.get(), inputs_tools);

        assert_equals(COMMON_CHAT_FORMAT_PEG_NATIVE, syntax_no_tools.format);
        assert_equals(COMMON_CHAT_FORMAT_PEG_NATIVE, syntax_tools.format);

        // Functionary v3.2 uses >>> prefix format - tests use template-generated parser
    ...
    }
    {
        ...
        // DeepSeek R1 uses template-generated parser - standalone parsing tests removed
        // (these relied on built-in format-specific parsers which are no longer supported)
    }
    {
        ...
        // Standalone parsing tests removed - all templates use auto-parser now
        test_templates(tmpls.get(), end_tokens, message_assist_call, tools,
    ...
    if (false) {
        ...
        assert_msg_equals(message_assist, common_chat_parse("Hello, world!\nWhat's up?",
                                                            /* is_partial= */ false, { COMMON_CHAT_FORMAT_PEG_NATIVE }));
        assert_msg_equals(message_assist, common_chat_parse("Hello, world!\nWhat's up?",
                                                            /* is_partial= */ true, { COMMON_CHAT_FORMAT_PEG_NATIVE }));
    ...
    if (false) {
        ...
        }
        {
            ...
            for (std::size_t i = std::string("<seed:tool_call>").length(); i < tool_msg.length() - 1; i++) {
                ...
                auto partial_res =
                    common_chat_parse(partial, true, { COMMON_CHAT_FORMAT_PEG_NATIVE, COMMON_REASONING_FORMAT_DEEPSEEK });
...
<|im_start|>assistant
...

        {
            ...
            test_parser_with_streaming(
                ...
                "</tool_call>",
                [&](const std::string & msg) {
                    return common_chat_parse(msg, /* is_partial= */ true, { COMMON_CHAT_FORMAT_PEG_NATIVE });
                });
            test_parser_with_streaming(
...

        {
            ...
            test_parser_with_streaming(multi_tool_msg,
                                       ...
                                       [&](const std::string & msg) {
                                           return common_chat_parse(
                                               msg, /* is_partial= */ true,
                                               { COMMON_CHAT_FORMAT_PEG_NATIVE, COMMON_REASONING_FORMAT_DEEPSEEK });
                                       });
            test_parser_with_streaming(
...
static void test_template_output_peg_parsers() {
...

    {
        ...
        tst.test("[THINK]I'm\nthinking[/THINK]" R"([TOOL_CALLS]special_function[ARGS]{"arg1":1})")
            .reasoning_format(COMMON_REASONING_FORMAT_AUTO)
            .tools({ special_function_tool })
        ...
        tst.test("[THINK]I need to output the invoice details in JSON[/THINK]"
...

    {
        ...
        tst.test("<|START_THINKING|><|END_THINKING|>"
                 "<|START_ACTION|>[\n"
                 "    {\"tool_call_id\": \"0\", \"tool_name\": \"special_function\", \"parameters\": {\"arg1\": 1}}\n"
                 "]<|END_ACTION|>")
            .reasoning_format(COMMON_REASONING_FORMAT_DEEPSEEK)
...

    {
        ...
        tst.test("<|tool_call|>[{\"name\": \"special_function\", \"arguments\": {\"arg1\": 1}}]")
            .tools({ special_function_tool })
            .expect(message_assist_call)
            .run();

        tst.test("<|tool_call|>[{\"name\": \"special_function\"")
            .tools({ special_function_tool })
            .is_partial(true)
            .expect(message_assist_call_empty_args)
            .run();
...

    {
        ...
        tst.test("<seed:think>I need to call a function</seed:think>"
                 "<seed:tool_call>\n"
                 "<function=special_function>\n"
                 "<parameter=arg1>1</parameter>\n"
                 "</function>\n"
    ...
    // TODO: deepseek-ai-DeepSeek-V3.1 tests are disabled - autoparser incorrectly detects reasoning markers
    // The template uses </think> as reasoning end marker, but autoparser detects it as start marker
    // because the prompt ends with </think> when thinking is disabled.
    // {
    ...
    // The ARGS_KEY_VALUE_TAGS format is partially implemented but the argument extraction
    // doesn't work correctly yet. The issue is in how the parser handles the bare function
    // name followed by key-value pairs.
    // {
    ...
    // Kimi-K2-Thinking tests - now supported with FUNC_PREFIXED_INDEXED format
    {
        ...
        tst.test("<|tool_calls_section_begin|><|tool_call_begin|>functions.special_function:0<|tool_call_argument_begin|>{\"arg1\": 1}<|tool_call_end|><|tool_calls_section_end|>")
            .tools({ special_function_tool })
            .expect(message_assist_call)
            .run();
    }
    ...
    {
        auto tst = peg_tester("models/templates/MiniMax-M2.jinja");
        tst.test("<minimax:tool_call>\n<invoke name=\"special_function\"><parameter name=\"arg1\">1</parameter></invoke>\n</minimax:tool_call>")
            .tools({ special_function_tool })
            .expect(message_assist_call)
            .run();
    }
