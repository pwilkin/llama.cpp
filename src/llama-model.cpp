    ...
    // create tensors for the weights
    {
        ...
        switch (arch) {
            ...
            case LLM_ARCH_DEEPSEEK2:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // Always try to load DSA weights if they exist in the model file
                        // The --dsa flag controls whether DSA is used at runtime
                        layer.wq_idx_a = create_tensor(tn(LLM_TENSOR_ATTN_Q_IDX_A, "weight", i),
                                                       { n_embd, kv_lora_rank }, TENSOR_NOT_REQUIRED);
                        layer.wq_idx_b = create_tensor(
                            tn(LLM_TENSOR_ATTN_Q_IDX_B, "weight", i),
                            { kv_lora_rank, hparams.n_dsa_indexer_heads * hparams.n_dsa_indexer_head_dim },
                            TENSOR_NOT_REQUIRED);
                        layer.dsa_idx_norm    = create_tensor(tn(LLM_TENSOR_ATTN_Q_IDX_NORM, "weight", i),
                                                              { kv_lora_rank }, TENSOR_NOT_REQUIRED);
                        layer.dsa_idx_weights = create_tensor(tn(LLM_TENSOR_ATTN_DSA_IDX_WEIGHTS, "weight", i),
                                                              { hparams.n_dsa_indexer_heads }, TENSOR_NOT_REQUIRED);
            ...
            case LLM_ARCH_WAVTOKENIZER_DEC:
                {
                    tok_embd =
                        create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), { hparams.n_embd_features, n_vocab }, 0);

                    conv1d   = create_tensor(tn(LLM_TENSOR_CONV1D, "weight"),
                                             { 7, hparams.n_embd_features, hparams.posnet.n_embd }, 0);
